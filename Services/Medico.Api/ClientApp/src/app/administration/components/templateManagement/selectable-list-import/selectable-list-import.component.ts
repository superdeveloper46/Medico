...
export class SelectableListImportComponent implements OnInit {
  ...
  @Output() selectableListImportApplied = new EventEmitter<void>();

  @Output() selectableListImportCanceled = new EventEmitter<void>();
  ...
  cancelListsImporting() {
    this.selectableListImportCanceled.next();
  }
  ...
  importLists() {
    ...
    const selectedListsIds = this.selectedLists.map(t => t.id || '');

    this.selectableListService
      .importLibrarySelectableLists(this.companyId, this.categoryId, selectedListsIds)
      .then(() => {
        this.selectableListImportApplied.next();
      });
  }
