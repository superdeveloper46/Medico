...
export class DragAndDropListComponent {
  @Input()
  listItems!: SortableItem[];

  @Output()
  orderChanged: EventEmitter<SortableItem[]> = new EventEmitter();

  get listItemsEmpty(): boolean {
    return !this.listItems || !this.listItems.length;
  }

  get firstItemInList(): Nullable<SortableItem> {
    return this.listItemsEmpty ? null : this.listItems[0];
  }
  ...
  onTemplatesReordered($event: any) {
    const fromIndex = $event.fromIndex;
    const toIndex = $event.toIndex;

    const increaseOrder = toIndex > fromIndex;

    const shiftNumber = increaseOrder ? toIndex - fromIndex : fromIndex - toIndex;
    ...
    if (increaseOrder) {
      this.listItems.forEach(listItem => {
        ...
        if (isOrderShouldBeChanged) listItem.order = listItem.order - 1;
      });
    } else {
      this.listItems.forEach(listItem => {
        const isOrderShouldBeChanged =
    ...
    this.orderChanged.next(this.listItems);
  }
}
