...
export class ExpressionImportComponent implements OnInit {
  ...
  @Output() expressionImportApplied: EventEmitter<void> = new EventEmitter<void>();

  @Output() expressionImportCanceled: EventEmitter<void> = new EventEmitter<void>();
  ...
  cancelExpressionsImporting() {
    this.expressionImportCanceled.next();
  }
  ...
  importExpressions() {
    ...
    const selectedExpressionIds = this.selectedExpressions.map(t => t.id);

    this.expressionService
      .importLibraryExpressions(this.companyId, selectedExpressionIds)
      .then(() => {
        this.expressionImportApplied.next();
      });
  }
