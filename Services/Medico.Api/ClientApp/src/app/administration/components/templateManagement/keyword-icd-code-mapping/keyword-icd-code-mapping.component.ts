...
export class KeywordIcdCodeMappingComponent
  extends BaseAdminComponent
  implements AfterViewInit
{
  @Input() companyId?: string;

  @ViewChild('codeKeywordsDataGrid', { static: false })
  codeKeywordsDataGrid!: DxDataGridComponent;
  @ViewChild('codeKeywordsPopup', { static: false })
  codeKeywordsPopup!: DxPopupComponent;
  @ViewChild('codeKeywordsForm', { static: false })
  codeKeywordsForm!: DxFormComponent;

  isNewCodeKeywordsMapping = true;
  keywordIcdCodes: any;
  ...
  isCodeKeywordsPopupOpened = false;
  ...
  saveCodeKeywords() {
    const validationResult = this.codeKeywordsForm.instance.validate();
    ...
    Promise.all(saveCodeKeywordPromises).then(() => {
      this.isCodeKeywordsPopupOpened = false;
    });
  }

  validateIcdCodeMappingExistence(params: any) {
    if (!this.isNewCodeKeywordsMapping) return true;

    const icdCodeId = params.value;
    this.chiefComplaintKeywordService.getByIcdCode(icdCodeId).then(keywords => {
  ...
  validateKeywordExistence(params: any) {
    const keywordValue = params.value.toUpperCase();
    const keywordVWithSameValue = this.keywords.filter(l => l.value === keywordValue)[0];
  ...
  onNewKeywordInserted($event: any) {
    if (!this.isNewCodeKeywordsMapping) {
  ...
  onKeywordRemoved($event: any) {
    if (!this.isNewCodeKeywordsMapping) {
      ...
      this.chiefComplaintKeywordService
        .deleteIcdCodeMap(keywordId, this.codeKeywordsFormData.icdCode)
        .catch(error => this.alertService.error(error.message ? error.message : error));
    } else {
      const keywordValue = $event.data.Value;
      if (!keywordValue) return;

      const keywordInedxToRemove = this.keywords.map(k => k.value).indexOf(keywordValue);
