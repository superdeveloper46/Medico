            ...
            IPatientChartDocumentNodeService
    {
            ...
            IDictionary<Guid, Guid> templatesMap, IDictionary<Guid, Guid> templateTypesMap)
        {
            ...
            foreach (var libraryDocument in libraryDocumentNodes)
            {
                ...
                await Repository.AddAsync(companyPatientChartDocument);
            }
        }
        ...
        private static bool DoesDocumentContainTemplateListNode(Guid templateTypeId,
        ...
        public async Task<List<PatientChartDocumentNode>> GetByCompanyId(Guid companyId)
        {
            var patientChartDocumentNode = await Repository.GetAll().Where(x => x.CompanyId == companyId).ToListAsync();
            return patientChartDocumentNode;
        }
