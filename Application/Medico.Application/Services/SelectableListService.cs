        ...
        : BaseDeletableByIdService<SelectableList, SelectableListVm>, ISelectableListService
    {
        ...
        public IQueryable<CompanyCategorySelectableListVm> Grid(CompanyDxOptionsViewModel loadOptions)
        {
            ...
            return _selectableListViewRepository.GetAll().Where(l => l.CompanyId == companyId)
                .ProjectTo<CompanyCategorySelectableListVm>(_mapper.ConfigurationProvider);
        }
            ...
            IDictionary<Guid, Guid> selectableListCategoriesMap)
        {
            ...
            foreach (var librarySelectableList in librarySelectableLists)
            {
                ...
                await Repository.AddAsync(companySelectableList);
            }
        ...
        public Task<List<SelectableListVm>> GetByFilter(SelectableListSearchFilterVm searchFilter)
        {
            ...
            return query
                .ProjectTo<SelectableListVm>(_mapper.ConfigurationProvider)
                .ToListAsync();
        }
        ...
        public IQueryable<CategorySelectableListVm> LibraryGrid(DxOptionsViewModel loadOptions)
        {
            return _selectableListViewRepository.GetAll().Where(l => l.CompanyId == null)
                .ProjectTo<CategorySelectableListVm>(_mapper.ConfigurationProvider);
        }
        ...
        public async Task<IEnumerable<Docs>> GetDocList()
        {
            IDbConnection con = Connection;
            con.Open();
            var chartData =
              await con.QueryAsync<Docs>(@"
                select JsonValues from SelectableList Where Title='DOC_NOTES_SUBJECT' and IsActive=1",
              new
              { }, commandType: CommandType.Text);
            return chartData;
        }

        public async Task<IEnumerable<Docs>> GetMessageStatusList()
        {
            IDbConnection con = Connection;
            con.Open();
            var chartData =
              await con.QueryAsync<Docs>(@"
                select JsonValues from SelectableList Where Title='MessageStatus' and IsActive=1",
              new
              { }, commandType: CommandType.Text);
            return chartData;
        }

        public async Task<IEnumerable<Docs>> GetCategoryList()
        {
            IDbConnection con = Connection;
            con.Open();
            var chartData =
              await con.QueryAsync<Docs>(@"
                select JsonValues from SelectableList Where Title='Lab_Test_Category' and IsActive=1",
              new
              { }, commandType: CommandType.Text);
            return chartData;
        }

        public async Task<IEnumerable<TemplateWithTypeNameViewModel>> GetProcedureList(Guid companyId)
        {
            try
            {
                IDbConnection con = Connection;
                con.Open();
                var chartData =
                  await con.QueryAsync<TemplateWithTypeNameViewModel>(@"
                select ReportTitle from Template where  Title like 'PROC_%' order by ReportTitle",
                  new { companyId }, commandType: CommandType.Text);
                return chartData;
            }
            catch(Exception ex)
            {
        ...
        public async Task<IEnumerable<Docs>> GetDiagnosisList()
        {
            IDbConnection con = Connection;
            con.Open();
            var chartData =
                await con.QueryAsync<Docs>(@"
                    select JsonValues from SelectableList Where Title='Diagnostic_Code' and IsActive=1",
                new
                { }, commandType: CommandType.Text);
            return chartData;
        }

        public async Task<IEnumerable<Docs>> GetMessageTypeList()
        {
            IDbConnection con = Connection;
            con.Open();
            var chartData =
                await con.QueryAsync<Docs>(@"
                    select JsonValues from SelectableList Where Title='MessageType' and IsActive=1",
                new
                { }, commandType: CommandType.Text);
            return chartData;
        }

        public async Task<IEnumerable<Docs>> GetMessageSubjectList()
        {
            IDbConnection con = Connection;
            con.Open();
            var chartData =
                await con.QueryAsync<Docs>(@"
                    select JsonValues from SelectableList Where Title='MessageSubject' and IsActive=1",
                new
                { }, commandType: CommandType.Text);
            return chartData;
        }

        public async Task<IEnumerable<Docs>> GetSubTaskTitleList()
        {
            IDbConnection con = Connection;
            con.Open();
            var chartData =
                await con.QueryAsync<Docs>(@"
                    select JsonValues from SelectableList Where Title='SubTaskTitle' and IsActive=1",
                new
                { }, commandType: CommandType.Text);
            return chartData;
        }

        public async Task<IEnumerable<Docs>> GetErrorLogStatusList()
        {
            IDbConnection con = Connection;
            con.Open();
            var chartData =
                await con.QueryAsync<Docs>(@"
                    select JsonValues from SelectableList Where Title='ErrorLogStatus' and IsActive=1",
                new
                { }, commandType: CommandType.Text);
            return chartData;
        }
