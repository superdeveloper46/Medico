    ...
    public class IcdCodeService : IIcdCodeService
    {
        ...
        public IQueryable<IcdCodeViewModel> GetAll()
        {
            return _icdCodeRepository.GetAll()
                .ProjectTo<IcdCodeViewModel>(_mapper.ConfigurationProvider);
        }
        ...
        public IQueryable<IcdCodeViewModel> GetAllForLookup(DxOptionsViewModel dxOptions, int lookupItemsCount)
        {
            ...
            return _icdCodeRepository.GetAll()
                .Where(c => EF.Functions.Like(c.Name, $"%{searchIcdCodeString}%") ||
                            EF.Functions.Like(c.Code, $"%{searchIcdCodeString}%"))
                .ProjectTo<IcdCodeViewModel>(_mapper.ConfigurationProvider);
        }
        ...
        public async Task<IEnumerable<IcdCodeViewModel>> Search(string keyword)
        {
            var mappedIcdCodes = await _icdCodeRepository
               .GetAll()
               .Take(100)
               .ToListAsync();

            var records = from icdCode in mappedIcdCodes
                          select new IcdCodeViewModel
                          {
                              Id = icdCode.Id,
                              Code = icdCode.Code,
                              Name = $"{icdCode.Code}-{icdCode.Name}",
                              Notes = icdCode.Notes
                          };
            return records;
        }
