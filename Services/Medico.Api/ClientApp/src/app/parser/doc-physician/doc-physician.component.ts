...
@Component({
  selector: 'app-doc-physician',
  templateUrl: './doc-physician.component.html',
  styleUrls: ['./doc-physician.component.sass'],
})
export class DocPhysicianComponent {
  ...
  previewData(ext: string) {
    ...
    this.repository.create(apiUrl, this.claimants[0]).subscribe({
      next: res => {
        if (res.success) {
          // state
          const st = this.states.filter(c => c.name == res.data.stateName)[0];
  ...
  initDocumentSave(parserId: string) {
    ...
    const apiUrl = `dataParser/physicianDoc/company/${this.companyId}/${parserId}`;
  ...
  onPatientFieldChanged(_$event: any) {
    // const dataField = $event.dataField;
    // if (dataField === "zipCodeType" && $event.value)
    //     this.patient.zip = "";
  }

  onPatientInsuranceFieldChanged(_$event: any) {
    // const dataField = $event.dataField;
    // if (dataField === "zipCodeType" && $event.value) {
