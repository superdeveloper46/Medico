...
@Component({
  selector: 'app-patient-chart-pre-auth',
  templateUrl: './patient-chart-pre-auth.component.html',
  styleUrls: ['./patient-chart-pre-auth.component.sass'],
})
export class PatientChartPreAuthComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngAfterViewInit() {
    this.service
      ...
      .then((x: any) => {
        this.responseData = x;
        if (x.success) {
          this.nodeData = x.data.preAuth ? JSON.parse(x.data.preAuth)[0] : null;
        }
        if (this.nodeData) {
          ...
          this.nodeData = this.getPatientChartTreeProjection(
            this.nodeData,
            patientChartTreeViewCopy[0],
            false
          );
        }
  ...
  getPatientChartTreeProjection(
    patientChartNode: any,
    patientChartNodeFromAdmission: any,
    isSignedOff: boolean
  ): PatientDataModelNode {
    if (patientChartNode.text == 'History of Present Illness') {
    }
    ...
    //   visible = false;
    // }
    if (!patientChartNodeFromAdmission) {
    }
    const id = patientChartNodeFromAdmission.id;
    const text = patientChartNode.text;
    const name = patientChartNode.name;
    const expanded = patientChartNode.expanded;
    const isNotShownInReport =
      patientChartNode.selected !== undefined ? patientChartNode.selected : true;
    visible = true;
    // patientChartNode.selected !== undefined
    //   ? patientChartNode.selected
    //   : true;

    let projectionTreeNode = new PatientDataModelNode(
      id,
      text,
      name,
      expanded,
    ...
    projectionTreeNode =
      patientChartNode.selected !== undefined
        ? { ...projectionTreeNode, selected: patientChartNode.selected }
        : projectionTreeNode;

    const nodeChildrens2 = patientChartNodeFromAdmission.items;
    const nodeChildrens = patientChartNode.items;
  ...
  patientChartSectionSelectionChanged($event: any) {
    ...
    if (isSectionSelected) this.addPatientChartNodeReportContent(reportTreeNode.id, true);
  ...
  private getReportHeader(): Promise<string> {
    if (!this.admission?.id) return Promise.resolve('');
  ...
  private generateReportBody(): ReportDataTreeNode {
    // const patientChartTree = this.patientChartTreeViewComponent.dataSource[0];

    const patientChartTree = this.nodeData;
  ...
  private convertPatientChartTreeNodeToReportDataTreeNode(
    ...
    if (patientChartTreeNodeChildren && patientChartTreeNodeChildren.length) {
      for (let i = 0; i < patientChartTreeNodeChildren.length; i++) {
        ...
        const childDataTreeNodetemp = {
          ...childDataTreeNode,
          selected: patientChartTreeChildNode.selected,
        };

        reportDataTreeNode.childrenNodes.push(childDataTreeNodetemp);
      }
    }
  ...
  private addPatientChartNodeReportContent(patientTreeNodeId: string, isChanges = false) {
    if (!this.reportDataTree) return;
    ...
    if (isChanges) {
      reportDataTreeNode.selected = true;
    }
  ...
  private addPatientChartTreeReportContent(
    ...
    return this.reportSectionService
      ...
      .then(htmlContent => {
        if (htmlContent)
          reportDataTreeNode.html = reportDataTreeNode.selected ? htmlContent : '';
  ...
  private initPatientChartTreeViewComponentDataSource() {
    ...
    this.patientChartTreeView[0].expanded = true;
    const patientChartTreeViewCopy = ObjectHelper.copy(this.patientChartTreeView);
    this.data = this.nodeData = this.nodeData
      ? this.nodeData
      : this.getPatientChartReportTreeViewNode(patientChartTreeViewCopy[0]);
    this.patientChartTreeViewComponent.dataSource = [this.data];
  }
