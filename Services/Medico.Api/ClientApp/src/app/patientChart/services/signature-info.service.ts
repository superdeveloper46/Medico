...
export class SignatureInfoService {
  ...
  getSignatureString(admissionId: string): Promise<string> {
    ...
    return firstValueFrom(this.http.get<SignatureInfo>(url)).then(signatureInfo => {
      ...
      return this.userService.getById(signatureInfo.physicianId).then(physician => {
  ...
  save(signatureInfo: SignatureInfo): Promise<void> {
    return firstValueFrom(
      this.http.post<void>(`${this.config.apiUrl}signatureinfo/`, signatureInfo)
    );
  }

  getSignature(admissionId: string): Promise<SignatureInfo> {
    const url = `${this.config.apiUrl}signatureinfo/existing/admission/${admissionId}`;
    return firstValueFrom(this.http.get<SignatureInfo>(url)).then(signatureInfo => {
