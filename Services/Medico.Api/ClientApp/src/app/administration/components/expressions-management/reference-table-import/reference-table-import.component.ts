...
export class ReferenceTableImportComponent implements OnInit {
  ...
  @Output() referenceTableImportApplied: EventEmitter<void> = new EventEmitter<void>();

  @Output() referenceTableImportCanceled: EventEmitter<void> = new EventEmitter<void>();
  ...
  cancelReferenceTablesImporting() {
    this.referenceTableImportCanceled.next();
  }
  ...
  importReferenceTables() {
    ...
    const selectedReferenceTableIds = this.selectedReferenceTables.map(t => t.id);

    this.referenceTableService
      .importLibraryReferenceTables(this.companyId, selectedReferenceTableIds)
      .then(() => {
        this.referenceTableImportApplied.next();
      });
  }
