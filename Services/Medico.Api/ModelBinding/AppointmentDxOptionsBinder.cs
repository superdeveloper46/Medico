...
namespace Medico.Api.ModelBinding
{
    public class AppointmentDxOptionsBinder : BaseDxOptionsBinder
    {
        protected override DataSourceLoadOptionsBase GetLoadOptions(ModelBindingContext bindingContext)
        {
            ...
            appointmentLoadOptions.AppointmentStatuses = 
                GetUrlParameterValue(bindingContext, "AppointmentStatuses", s => s);
            
            appointmentLoadOptions.FilterType = 
                GetUrlParameterValue(bindingContext, "FilterType", s => s == null ? null : (FilterType?)int.Parse(s));
