...
export class AdminSelectableDateComponent
  ...
  implements IAdminSelectableComponent
{
  ...
  selectedDateFormatId: number = this.dateFormats[0].id;
  ...
  addSelectableItemGeneratedValue() {
    if(this.selectedDateFormatId == 4)
    {
      this.selectableHtmlElementGenerated.next("<label var-id='var_today' contenteditable='false'>var: Today</label>");
    }
    else {
      const selectedFormat = this.dateFormats.filter(
        df => df.id === this.selectedDateFormatId
      )[0].format;
