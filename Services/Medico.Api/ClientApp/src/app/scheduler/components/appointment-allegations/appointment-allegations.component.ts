...
export class AppointmentAllegationsComponent implements AfterViewInit, OnInit {
  ...
  @ViewChild('allegationForm', { static: false })
  allegationForm!: DxFormComponent;
  @ViewChild('allegationPopup', { static: false })
  allegationPopup!: DxPopupComponent;
  ...
  ngAfterViewInit(): void {
    this.allegationPopup.instance.registerKeyHandler('escape', (event: any) => {
  ...
  onAllegationSelected($event: any) {
    let selectedAllegation = $event.addedItems[0];

    if (!selectedAllegation) return;
  ...
  deleteMappedIcdCode(icdCode: any, $event: any) {
    ...
    this.alertService
      .confirm(
        'Are you sure you want to delete icd code from mapping?',
        'Confirm deletion'
      )
      .then(deleteConfirmation => {
        if (deleteConfirmation) {
          ...
          this.icdCodeService
            .deleteIcdCodeMapping(this.selectedAllegation, icdCodeId)
            .then(() => {
              this.setMappedKeywords();
            })
            .catch(error =>
  ...
  setAllegations() {
    this.allegations = this.allegationsString ? this.allegationsString.split(',') : [];
  }
  ...
  createAllegationIcdCodeMapping() {
    ...
    this.chiefComplaintKeywordService.createIcdCodeMap(
      this.newAllegationIcdCodeMapping.keywordValue,
  ...
  validateMappingExistence(params: any) {
    const icdCodeId = params.value;

    this.icdCodeService
      .checkMappingExistence(icdCodeId, this.selectedAllegation)
      .then(isMappingExist => {
  ...
  private setMappedKeywords() {
    this.icdCodeService
      .getMappedToKeword(this.selectedAllegation)
      .then(mappedIcdCodes => {
