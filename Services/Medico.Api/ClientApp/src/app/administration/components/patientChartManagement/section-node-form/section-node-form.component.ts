...
export class SectionNodeFormComponent implements OnInit {
  @ViewChild('sectionNodeForm', { static: false })
  sectionForm!: DxFormComponent;

  @Input() patientChartContextMenuAction?: PatientChartContextMenuAction;

  @Output()
  sectionNodeEdited: EventEmitter<PatientChartNodeChanges> = new EventEmitter<PatientChartNodeChanges>();

  @Output()
  sectionNodeAdded: EventEmitter<PatientChartNode> = new EventEmitter<PatientChartNode>();

  sectionNode: any = { title: '' };
  ...
  createUpdateSectionNode() {
    const validationResult = this.sectionForm.instance.validate();

    if (!validationResult.isValid) return;

    const newSectionNodeTitle = this.sectionNode.title;
    const newSectionNodeName =
      this.entityNameService.formatFromReadableEntityName(newSectionNodeTitle);
  ...
  private processSectionNodeAdding(
    newSectionNodeTitle: string,
    newSectionNodeName: string
  ) {
    ...
    const newDocumentNode = PatientChartNode.createPatientChartNode(
      ...
      null,
      newSectionNodeAttributes,
      newSectionNodeParentId,
      ''
    );
    ('');
    this.sectionNodeAdded.next(newDocumentNode);
  }

  private processSectionNodeEditing(
    newSectionNodeTitle: string,
    newSectionNodeName: string
  ) {
    ...
    this.sectionNodeEdited.next(sectionNodeChanges);
  }
}
