  ...
  implements OnInit, OnDestroy
{
  ...
  isSelectableListImportFormVisible = false;
  ...
  onSelectableListImportApplied() {
    this.selectableListDataGrid.instance.getDataSource().reload();
  ...
  createUpdateSelectableList() {
    ...
    const data = this.selectableList;
    this.selectableListService
      .getByTitle(data.title, this.companyId)
      .then(selectableList => {
        ...
        if (!isSelectableListTitleValid) {
          this.alertService.warning(
            `Selectable list with title '${data.title}' already exists`
          );
          return;
        } else {
  ...
  syncWithLibrarySelectableList(selectableList: SelectableList, $event: any) {
    ...
    const confirmationPopup = this.alertService.confirm(
      'Are you sure you want to sync selectable list ?',
      'Confirm sync'
    );
    ...
    confirmationPopup.then(dialogResult => {
      if (dialogResult) {
        this.selectableListService
          .syncWithSelectableListTemplate(selectableListId, selectableList.version)
          .then(() => {
            this.selectableListDataGrid.instance.refresh();
