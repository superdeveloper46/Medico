...
export class TemplateListNodeFormComponent implements OnInit {
  @ViewChild('templateListNodeForm', { static: false })
  templateListNodeForm!: DxFormComponent;

  @Input() patientChartContextMenuAction?: PatientChartContextMenuAction;
  @Input() companyId!: string;
  @Input() isLibraryManagement!: boolean;

  @Output() templateListNodeEdited = new EventEmitter<PatientChartNodeChanges>();

  @Output() templateListNodeAdded = new EventEmitter<PatientChartNode>();
  ...
  isTemplateListNodeFormShown = false;
  ...
  createUpdateTemplateListNode() {
    const validationResult = this.templateListNodeForm.instance.validate();
      ...
      .then(templateType => {
        if (this.isEditMode) {
          if (this.patientChartContextMenuAction) {
            this.processTemplateNodeEditing(
              newTemplateListNodeTitle,
              templateType,
              this.patientChartContextMenuAction.patientChartTreeItem
            );
          }
        } else {
  ...
  private setupTemplateListNode() {
    if (this.isEditMode) {
      const templateTypeName =
        this.patientChartContextMenuAction?.patientChartTreeItem.name;
      if (!templateTypeName) return;

      this.templateTypeService
        ?.getByName(templateTypeName, this.companyId)
        .then((templateType: any) => {
          this.templateListNode.templateTypeId = templateType.id;
          this.templateListNode.title =
  ...
  private processSectionNodeAdding(
    newTemplateListNodeTitle: string,
    templateType: TemplateType
  ) {
    const newTemplateListNodeParentId =
      this.patientChartContextMenuAction?.patientChartTreeItem.id;

    const newTemplateListNodeId = GuidHelper.generateNewGuid();
    ...
    newTemplateListNodeAttributes.nodeSpecificAttributes = nodeSpecificAttributes;

    const newTemplateListNode = PatientChartNode.createPatientChartTemplateListNode(
      newTemplateListNodeId,
      templateType.name,
      newTemplateListNodeTitle,
      newTemplateListNodeAttributes,
      newTemplateListNodeParentId
    );

    this.templateListNodeAdded.next(newTemplateListNode);
  }

  private processTemplateNodeEditing(
    newTemplateListNodeTitle: string,
    templateType: TemplateType,
    templateListNode: PatientChartAdminNode
  ) {
    const templateListNodeChanges = new PatientChartNodeChanges();
    ...
    const newNodeAttributes = PatientChartNodeAttributes.createPatientChartNodeAttributes(
      templateListNode.order,
      templateListNode.isActive,
      false,
      false,
      templateListNode.isPredefined,
      {
        templateTypeId: templateType.id,
      }
    );
    ...
    this.templateListNodeEdited.next(templateListNodeChanges);
  }

  private initTemplateTypeService(isLibraryManagement: boolean, injector: Injector) {
    this.templateTypeService = injector.get<BaseTemplateTypeService>(
      isLibraryManagement ? LibraryTemplateTypeService : TemplateTypeService
    );
  }
}
