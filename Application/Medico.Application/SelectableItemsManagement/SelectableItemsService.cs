...
namespace Medico.Application.SelectableItemsManagement
{
    public class SelectableItemsService : ISelectableItemsService
    {
        ...
        public string WrapAllSelectableListValuesToBoldTag(string detailedTemplateContent)
        {
            ...
            {
                selectableNode.InnerHtml = $"<b>{selectableNode.InnerHtml}</b>";
            }
        ...
        public SelectableVariables GetSelectableVariablesFromHtmlContent(string htmlContent, bool isForTesting = false)
        {
            ...
            var selectableVariables = GetSelectableVariables(htmlContent);

            if (!selectableVariables.Any())
                return null;
        ...
        public VariablesUniquenessCheckResultVm CheckNonUniqueSelectableVariablesInHtmlContent(string htmlContent)
        {
            var variablesUniquenessCheckResultVm =
                new VariablesUniquenessCheckResultVm();

            var selectableVariables = GetSelectableVariables(htmlContent);
            if (!selectableVariables.Any())
                return variablesUniquenessCheckResultVm;

            var groupedNonUniqueVariables =
                selectableVariables.GroupBy(v => v.VariableName)
                    .Where(variablesByName => variablesByName.Count() > 1)
                    .ToList();

            if (!groupedNonUniqueVariables.Any())
                return variablesUniquenessCheckResultVm;

            foreach (var variablesByName in groupedNonUniqueVariables)
            {
                variablesUniquenessCheckResultVm
                    .NonUniqueVariableNames.Add(variablesByName.Key);
            }

            return variablesUniquenessCheckResultVm;
        }

        private List<SelectableVariable> GetSelectableVariables(string htmlContent)
        {
            var htmlDocumentString = string.Format(_htmlDocumentFormat, htmlContent);
            ...
            if (selectableVariableNodes == null || !selectableVariableNodes.Any())
                return Enumerable.Empty<SelectableVariable>()
                    .ToList();
