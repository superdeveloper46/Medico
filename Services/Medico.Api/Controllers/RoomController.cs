...
namespace Medico.Api.Controllers
{
    ...
    public class RoomController : ApiController
    {
        ...
        public async Task<IActionResult> Delete(Guid id)
        {
            ...
            if (!await CompanySecurityService.DoesUserHaveAccessToCompanyLocation(locationId))
                return Unauthorized();
        ...
        [HttpGet]
        [Route("ddl")]
        public object DropdownData(string companyId, string locationId)
        {
            RoomDxOptionsViewModel loadOptions = new RoomDxOptionsViewModel
            {
                CompanyId = new Guid(companyId),
                LocationId = new Guid(locationId)
            };
            loadOptions.PrimaryKey = new[] { "Id" };
            loadOptions.PaginateViaPrimaryKey = true;
