...
export class CompanySwitcherComponent implements OnInit, OnDestroy {
  @ViewChild('companiesSelectBox', { static: false })
  companiesSelectBox!: DxSelectBoxComponent;

  companyName: string = '';
  companyId: string = '';

  isCompanySelectBoxVisible = false;
  ...
  onCompanyListChanged($event: any): void {
  ...
  private subscribeToCompanyChanges() {
    this.subscription.add(
      this.companyCreateUpdateTrackService.companyChanges.subscribe(companyId => {
        if (
          companyId != GuidHelper.emptyGuid &&
          this.companiesSelectBox &&
          this.companiesSelectBox.instance
        ) {
  ...
  private subscribeToApplicationUserChanges() {
    this.subscription.add(
      this.authenticationService.currentUser.subscribe(currentUser => {
        ...
        if (isUserSuperAdmin) this.setCurrentCompanyId();
        else {
  ...
  private setCurrentCompanyId() {
    ...
    const isPatientChartUrl = currentUrl.indexOf(AppRouteNames.patientChart) !== -1;
    ...
    if (isPatientChartUrl) {
      ...
      const appointmentId = currentUrl.split('/')[2];

      companyPromise = this.companyService.getByAppointmentId(appointmentId);
    } else {
      companyPromise = this.companyService.getFirst();
    }
