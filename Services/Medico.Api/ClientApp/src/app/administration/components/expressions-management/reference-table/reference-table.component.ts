...
{
  @ViewChild('referenceTableDataGrid', { static: false })
  referenceTableDataGrid!: DxDataGridComponent;

  isReferenceTableImportFormVisible = false;
  ...
  onReferenceTableImportApplied() {
    this.referenceTableDataGrid.instance.getDataSource().reload();
  ...
  syncWithLibraryReferenceTable(referenceTable: ReferenceTable, $event: any) {
    ...
    const confirmationPopup = this.alertService.confirm(
      'Are you sure you want to sync reference table ?',
      'Confirm sync'
    );
    ...
    confirmationPopup.then(dialogResult => {
      if (dialogResult) {
        this.referenceTableService
          .syncWithLibraryReferenceTable(referenceTable.id, referenceTable.version)
          .then(() => {
