...
export class LibraryTemplateComponent extends BaseAdminComponent {
  ...
  @ViewChild('libraryTemplateTypeSelectBox', { static: false })
  libraryTemplateTypeSelectBox!: DxSelectBoxComponent;
  ...
  private _isDefaultTemplateEnabled = false;
  ...
  deactivateTemplate(template: Template, $event: any) {
    ...
    confirmationPopup.then(dialogResult => {
      if (dialogResult) {
        ...
        this.template.templateOrder = undefined;
        this.libraryTemplateService
          .activateDeactivateTemplate(template.id, false)
          .then(() => {
  ...
  activateTemplate(template: Template, $event: any) {
    $event.stopPropagation();
  ...
  deleteTemplate(template: Template, $event: any) {
    ...
    if (!template.id || !template.companyId) return;
  ...
  onlibraryTemplateTypeChanged($event: any): void {
    const libraryTemplateTypeId = $event.value;
    if (!libraryTemplateTypeId) return;

    this.libraryTemplateTypeService
      .getById(libraryTemplateTypeId)
