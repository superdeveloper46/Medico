...
export class TemplateContentCheckerService {
  ...
  findDuplicateWords(
    ...
    const selectableValues = this.getDistinctSelectableValuesFromHtml(
      templateContent,
    ...
    const templatesHtmlByType = this.templateContentService.getTemplatesContent(
      patientAdmissionModel,
      templateType
    );

    //selectable values that were found in all templates with specified type
    const selectableValuesByTemplateType = this.getDistinctSelectableValuesFromHtml(
      templatesHtmlByType,
      Constants.selectableItemTypes.list
    );

    if (!selectableValuesByTemplateType.length) return [];

    const response = this.getSimilarWords(selectableValues, selectableValuesByTemplateType);

    const duplicateWords = TextHelper.findDuplicateWords(templateContent);

    return duplicateWords;
  }

  private getSimilarWords(
    selectableValues1: string[],
    selectableValues2: string[]
  ): string[] {
    const similarWords = [];
  ...
  private getDistinctSelectableValuesFromHtml(
    htmlString: string,
    selectableType: string
  ): string[] {
    const selectableValuesFromHtml = this.selectableItemHtmlService
      .getSelectableItems(htmlString, [selectableType])
      .map(s => s.value);

    if (!selectableValuesFromHtml || !selectableValuesFromHtml.length) return [];

    return selectableValuesFromHtml.reduce((values: string[], value: string) => {
      const separatedByCommaValues = value.split(',');

      const trimmedValues = separatedByCommaValues.map(this.formatSelectableValue);
