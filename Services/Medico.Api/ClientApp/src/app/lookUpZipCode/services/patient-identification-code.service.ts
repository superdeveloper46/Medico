...
export class PatientIdentificationCodeService {
  ...
  FINexists(searchFilter: string, companyID: string): Promise<boolean> {
    return firstValueFrom(
      this.http.get<boolean>(
        `${this.config.apiUrl}${ApiBaseUrls.patientIdentificationCodes}/FINexists/${searchFilter}/${companyID}`
      )
    );
  }

  MRNexists(searchFilter: string, companyID: string): Promise<boolean> {
    return firstValueFrom(
      this.http.get<boolean>(
        `${this.config.apiUrl}${ApiBaseUrls.patientIdentificationCodes}/MRNexists/${searchFilter}/${companyID}`
      )
    );
  }

  LatterCheckedOrNot(companyID: string): Promise<boolean> {
    return firstValueFrom(
      this.http.get<boolean>(
        `${this.config.apiUrl}${ApiBaseUrls.patientIdentificationCodes}/LatterCheckedOrNot/${companyID}`
      )
    );
  }
}
